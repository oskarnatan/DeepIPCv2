...
class transfuser(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.gru = nn.GRUCell(input_size=6, hidden_size=64)
    ...
    def forward(self, image_list, lidar_list, rp1, rp2, velocity):
        '''
    ...
    def pid_control(self, pwaypoints, angular_velo):
        ...
        if pid_throttle < self.config.min_act_thrt:
            ...
            steering = float(pid_steering)
            throttle = float(pid_throttle)
...
class transfuser(nn.Module): #
    def __init__(self, config):
        ...
        self.RGB_encoder = models.resnet34(pretrained=True) #
        self.RGB_encoder.fc = nn.Sequential() #cara paling gampang untuk menghilangkan fc layer yang tidak diperlukan
        #------------------------------------------------------------------------------------------------
        #lidar
        self.lidar_encoder = models.resnet18(pretrained=False) #
        _tmp = self.lidar_encoder.conv1
        #input conv pertama diganti untuk menerima 2 channel histogram lidar
        self.lidar_encoder.conv1 = nn.Conv2d(2, _tmp.out_channels, kernel_size=_tmp.kernel_size, stride=_tmp.stride, padding=_tmp.padding, bias=_tmp.bias)
        self.lidar_encoder.fc = nn.Sequential() #cara paling gampang untuk menghilangkan fc layer yang tidak diperlukan
        self.lidar_encoder.apply(kaiming_init)
        #------------------------------------------------------------------------------------------------
        #feature fusion
    ...
    def forward(self, rgbs, pt_clds, rp1, rp2):
        #------------------------------------------------------------------------------------------------
        #bagian downsampling
        all_features_sum = 0
        for i in range(self.config.seq_len): #loop semua input dalam buffer            
            #encode rgb
            rgb_features = self.RGB_encoder(self.rgb_normalizer(rgbs[i]))
            #encode lidar
            lidar_features = self.lidar_encoder(pt_clds[i])
            #fusion: element-wise summation
            all_features_sum = all_features_sum + rgb_features + lidar_features
        #------------------------------------------------------------------------------------------------
        #waypoint prediction
        #get hidden state dari gabungan kedua bottleneck
        hx = self.necks_net(all_features_sum)
        # initial input car location ke GRU, selalu buat batch size x 2 (0,0) (xy)
        xy = torch.zeros(size=(hx.shape[0], 2)).to(self.config.gpu_device, dtype=hx.dtype)
        #predict delta wp
        out_wp = list()
